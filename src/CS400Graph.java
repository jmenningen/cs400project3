...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            this.start = copyPath.start;    // start is the same as copyPath
            this.distance = copyPath.distance + extendBy.weight; // add the weight of the edge to copyPath's distance

            // Make deep copy of copyPath.dataSequence
            this.dataSequence = new LinkedList<>();
            this.dataSequence.addAll(copyPath.dataSequence);

            this.dataSequence.add(extendBy.target.data); // add the end of the new edge to the data sequence
            this.end = extendBy.target; // set end to the end of the edge
        }
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        // Inner class for this method that tells: the vertex, whether or not it has been visited, Path to the vertex,
        ...
        class DijkstraVertex {
            ...
            public DijkstraVertex(T vertex) {
                this.vertex = vertices.get(vertex);
                this.path = new Path(vertices.get(vertex));
            }
        }
        // if any of the vertices aren't in the graph, then clearly no path
        if (!this.vertices.containsKey(start) || !this.vertices.containsKey(end))
            throw new NoSuchElementException();
        // Instantiate a DijkstraVertex with the start vertex
        DijkstraVertex startVertex = new DijkstraVertex(start);
        startVertex.totalWeight = 0; // set weight to 0 since it is the start vertex

        // Make hashtable of DijkstraVertex with key = data
        Hashtable<T, DijkstraVertex> dijkstraHashtable = new Hashtable<>(2*this.vertices.size());
        dijkstraHashtable.put(start, startVertex);

        // Instantiate a priority queue with the comparator being the path taking advantage of its compareTo method
        PriorityQueue<DijkstraVertex> pq = new PriorityQueue<>(2 * this.vertices.size(), Comparator.comparing(o -> o.path));

        pq.add(startVertex); // add the startVertex to the priority queue
        // While the priority queue is not empty
        while(!pq.isEmpty()) {
            DijkstraVertex currentVertex = pq.peek(); // currentVertex is the first thing in the priority queue
            currentVertex.visited = true;             // Change the currentVertex to visited

            // For every edge that the currentVertex contains
            for (Edge edge : currentVertex.vertex.edgesLeaving) {
                // If the hashtable doesn't contain the key add its DijkstraVertex
                if (!dijkstraHashtable.containsKey(edge.target.data))
                    dijkstraHashtable.put(edge.target.data, new DijkstraVertex(edge.target.data));

                DijkstraVertex newVertex = dijkstraHashtable.get(edge.target.data); // newVertex is the respective end of the edge

                // If the path from the current vertex is less than the path of the newVertex...
                if (newVertex.totalWeight > currentVertex.totalWeight + edge.weight) {
                ...
                // if the newVertex has not been visited, add it to the priority queue
                if (!newVertex.visited)
                    pq.add(newVertex);
            }
            // remove the currentVertex from the priority queue
            pq.remove(currentVertex);
        }

        // if the hashtable contains the key return the path, otherwise we can't reach the end from the start
        // so throw exception
        if (dijkstraHashtable.containsKey(end)) {
